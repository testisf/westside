...
export function DashboardPage({ onNavigate }: { onNavigate: (r: "servers" | "settings") => void }) {
  ...
  useEffect(() => {
    (async () => {
      ...
      const unlisten = await onUpdateAvailable((info: { version: string; date: string; body: string }) => {
  ...
  return (
    <div className="p-6 max-w-3xl">
      ...
      {updateAvailable && (
        <div className="mb-6 bg-primary/10 border border-primary/30 rounded-xl p-4">
          <div className="flex items-start justify-between gap-3">
            <div>
              <div className="font-medium text-sm text-primary">Update available — v{updateAvailable.version}</div>
              <p className="text-xs text-text-muted mt-1 whitespace-pre-line">{updateAvailable.body}</p>
            </div>
            <button
              onClick={handleInstallUpdate}
              disabled={installing}
              className="bg-primary hover:bg-primary-hover text-primary-foreground text-xs font-medium rounded-md px-3 py-1.5 disabled:opacity-50"
            >
      ...
      <div className="grid grid-cols-2 gap-4 mb-6">
        <StatCard label="Your servers" value={String(servers.length)} onClick={() => onNavigate("servers")} />
        <StatCard label="Permissions" value={String(me?.permissions.length ?? 0)} />
      </div>

      <section className="bg-surface border border-border rounded-xl p-5 mb-4">
        <h2 className="text-sm font-semibold mb-2">Phase 4 — Desktop client status</h2>
        <ul className="text-sm space-y-1 text-text-muted">
          <li>✓ Authentication with OS keychain storage</li>
          <li>✓ Two-factor (TOTP) login flow</li>
          <li>✓ Auto-update checker + signed manifest</li>
          <li>✓ Global PTT hotkey infrastructure (Ctrl+Shift+Space)</li>
          <li>✓ Servers list</li>
          <li>○ Phase 3 — Radio / PTT transmission (next)</li>
          <li>○ Phase 5 — MDT, Records, BOLOs, Warrants</li>
          <li>○ Phase 6 — ERLC/Roblox integration</li>
        </ul>
      </section>

      <section className="bg-surface border border-border rounded-xl p-5">
        <h2 className="text-sm font-semibold mb-2">Your servers</h2>
        {servers.length === 0 ? (
          <p className="text-sm text-text-muted">No servers yet. Create one to get started.</p>
        ) : (
      ...
      <div className="mt-6 text-xs text-text-muted">
        <button onClick={() => onNavigate("settings")} className="hover:underline">Settings →</button>
      </div>
    </div>
  );
}
...
function StatCard({ label, value, onClick }: { label: string; value: string; onClick?: () => void }) {
  return (
    <button
      onClick={onClick}
      disabled={!onClick}
      className={`bg-surface border border-border rounded-xl p-4 text-left ${onClick ? "hover:bg-surface-hover" : ""}`}
    >
